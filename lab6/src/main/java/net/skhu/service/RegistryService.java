...
public class RegistryService {
	...
	public List<Student> studentRecord(final InputStream excel)
		...
		for (int rowIndex = 0; rowIndex < rowMax; rowIndex++) { // 행을 돈다
			...
			for (int cellIndex = 0; cellIndex < 5; cellIndex++) { // 열을 돈다
				XSSFCell cell = row.getCell(cellIndex); // 셀 하나를 얻는다.
				if (cellIndex == 0) { // 학번
					switch (cell.getCellTypeEnum()) {
					case STRING:
						temp.setStudentId(cell.getStringCellValue());
						break;
					case NUMERIC:
					...
					switch (cell.getCellTypeEnum()) { //이름
					case STRING:
					...
					switch (cell.getCellTypeEnum()) {
					case STRING:
						temp.setPw(cell.getStringCellValue());
						break;
					case NUMERIC:
						temp.setPw((int) cell.getNumericCellValue() + "");
						break;
					default:
						break;
					}
				...
				System.out.println(temp.getStudentId() + " " + temp.getName() + " " + temp.getDepartmentId() + " "
						+ temp.getYear() + " " + temp.getPw());
